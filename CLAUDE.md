...
- [reviewr-tuicr-dark-2026-09-11]: herdr-reviewr's garish diff fill was reviewr's palette derivation, not Ghostty or herdr; fixed with a local fork adding a `tuicr-dark` theme
  - Root cause in `src/theme.rs` `derive`/`readable_tint`: every non-catppuccin theme tints `base` 20% toward its *text* green accent and keeps it whenever text contrast ≥ 4.5. Gruvbox's `#b8bb26` is chartreuse, so `ins_bg` = `#454528` khaki (lum 0.057) painted full-width on every `+` row (`ui.rs` `render_row`) over Ghostty's unpainted `#1d1f21` (lum 0.014). tuicr hand-pins `syntax_add_bg` per theme; its default dark uses `#00230c` (lum 0.012, darker than the terminal bg) with Base16 Eighties syntax. Ghostty's palette/`minimum-contrast` and herdr's `[theme]` cannot touch either app: both emit 24-bit SGR only, and reviewr reads only its own plugin config.
  - Fork at `~/code/personal/herdr-reviewr`, branch `tuicr-dark-theme` (`74b56e3` on upstream v0.36.2 `4c09022`): one new theme pinning tuicr's four fills over anchors derived from tuicr's accents, paired with `two-face` `Base16EightiesDark`; roster test + README updated. Plugin re-linked (`herdr plugin uninstall` → `herdr plugin link`), `~/.config/herdr/plugins/config/persiyanov.reviewr/config.toml` now `theme = "tuicr-dark"`. Upstream re-install would fall back to `catppuccin` with a log line, so a fresh machine needs the fork linked or the theme upstreamed.
  - **Machine finding**: rustup's default host on AVA is still `x86_64-apple-darwin` (Intel-era install) — `cargo build` produces Rosetta binaries. Built with `--target aarch64-apple-darwin` instead of changing the global host; `rustup set default-host aarch64-apple-darwin` + toolchain reinstall is the real fix, not done unilaterally. `just` is not installed; ran the `install` recipe's two commands directly.
  - Verified by tmux capture (`capture-pane -e`, `terminal-features ',*:RGB'`) against a throwaway repo: 7/8 `+` rows `48;2;0;35;12`, cursor row `surface2`, zero khaki, Base16 Eighties token colors present. **Trap**: the agent tool shell exports `NO_COLOR=1`, which crossterm honors and the tmux server inherits — every TUI color probe from a script must `env -u NO_COLOR`. Theme unit tests 8/8, `cargo fmt --check` clean. Toggle the reviewr pane off/on to pick up the new binary.
- [reviewr-fork-published-2026-09-11]: the reviewr patch now lives at `github.com/jryio/herdr-reviewr`; the plugin installs from its releases, not a local link
  - Supersedes the "fresh machine needs the fork linked" caveat above: `herdr plugin install jryio/herdr-reviewr` is now the install command everywhere, and `herdr plugin list` shows `github:jryio/herdr-reviewr@802822f`. Plugin id stays `persiyanov.reviewr` on purpose — herdr keys the config dir and `persiyanov.reviewr.toggle` action by id, so config and the `cmd+r` binding survived the swap.
